...
export default function Sessions({
  ...
  const handleClick = async () => {
    ...
    const { data: newSession } = await supabase
      ...
      .select();
    if (newSession) router.push(`/c/${newSession[0].id}`);
  };
  ...
  return (
    <>
      <button
        onClick={handleClick}
        className={
          "flex flex-row items-center justify-between w-full hover:bg-slate-800 p-2 rounded-xl"
        }
      >
        <div className={"flex flex-row items-center gap-2"}>
          <IconBrandOpenai size={24} /> FakeGPT
        </div>
        <div className={""}>
          <IconEdit size={18} />
        </div>
      ...
      <div className={"flex-1 overflow-scroll w-full"}>
        <ul className={"flex flex-col gap-2 w-full"}>
          {sessions.map((session) => (
            <div
              className={
                "flex flex-row justify-between hover:bg-zinc-800 w-full p-2 rounded-xl"
              }
              key={session.id}
            >
              <Link href={`/c/${session.id}`} prefetch={false}>
                {session.title}
              </Link>
